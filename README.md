...
8. Build an isolated Git index directly from `HEAD` and scan the complete non-ignored working tree before deciding whether changes exist. This exact prospective commit tree does not depend on an empty quick-status result and does not change the real staging area. Show the complete change list, then, after commit confirmation, verify that the repository, branch, HEAD, origin, account binding, Git index, file contents, and untracked-file set are still exactly the reviewed state.
